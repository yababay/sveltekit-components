...
export const getValueLine = (x: d3.ScaleTime<number, number, never>, y: d3.ScaleLinear<number, number, never>, data: [number, number][]) => {
    const fn = d3.line()
        .x(([ts]) => x(ts))
        .y(([_, v]) => y(v))
        .curve(d3.curveBasis)
...
export const getSubset = (records: ChartDataAttributes[], key: string):[number, number][] => {
...
const getLimits = (records: ChartDataAttributes[]) => {
...
export const setupDomainY = (records: ChartDataAttributes[], height: number): { yTemp: d3.ScaleLinear<number, number, never>, yPress: d3.ScaleLinear<number, number, never> } => {
...
export const getTimestamps = (records: ChartDataAttributes[]): Date[] => records.map(({ts}) => new Date(ts))

export const getValuableParams = (records: ChartDataAttributes[]): string[] => [...records.reduce((acc, rec) => {
    const keys = Reflect.ownKeys(rec)
    for (const key of keys){
        if (typeof key === "string" && /^\w+_\d+$/.test(key)) acc.add(key)
    }
    return acc
}, new Set<string>())]
.filter(name => records.reduce((acc, rec) => acc || Reflect.get(rec, name) !== 0, false))
.sort()
